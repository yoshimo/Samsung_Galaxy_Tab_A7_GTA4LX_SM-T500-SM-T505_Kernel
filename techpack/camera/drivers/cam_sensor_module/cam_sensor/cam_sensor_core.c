...
//+bug535065 Chris.wt, add, 2020.0313 ,add camera info interface for factory app
...
void cam_sensor_shutdown(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	//+bug549349, liuxingyu.wt, ADD, 2020/05/11 , add for cts testMultiCameraRelease
	if((s_ctrl->sensordata->slave_info.sensor_id == 0x487B) || (s_ctrl->sensordata->slave_info.sensor_id == 0x487C) || (s_ctrl->sensordata->slave_info.sensor_id == 0x4608))
		back_cam = 0;
	else if((s_ctrl->sensordata->slave_info.sensor_id == 0x0556) || (s_ctrl->sensordata->slave_info.sensor_id == 0x5035))
		front_cam = 0;
	//-bug549349, liuxingyu.wt, ADD, 2020/05/11 , add for cts testMultiCameraRelease
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (!slave_info) {
        ...
        if (slave_info->sensor_id == 0x5035) { //0x5035:n19_gc5035_front_qunhui
	...
	CAM_ERR(CAM_SENSOR, "module_id : 0x%x",module_id);
